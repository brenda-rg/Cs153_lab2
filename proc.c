...
fork(void)
{
  ...
  np->parent = curproc;
  *np->tf = *curproc->tf;
  np->prior_val = curproc->prior_val; // fork() child inherits parent's priority value
...
scheduler(void)
{
  ...
  struct cpu *c = mycpu();
  c->proc = 0;
  struct proc *ptemp; // temp var to change value in loop
  struct proc *pmost; //process with the highest priority (closest to 0**)
